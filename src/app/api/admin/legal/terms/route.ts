...
export async function GET() {
  ...
  if (!row) {
    return NextResponse.json({ ...TERMS_OF_SERVICE_DEFAULTS, updatedAt: 0, updatedBy: null });
  }
...
export async function PUT(req: NextRequest) {
  ...
  const content = str(body.content) || TERMS_OF_SERVICE_DEFAULTS.content;
  ...
  if (content.length > 50000) return NextResponse.json({ error: "El contenido es demasiado largo" }, { status: 400 });
