...
- **Eliminar servicios realizados sin cita:** nueva acción en `/dashboard/balances` para borrar filas de `service_purchases` con `appointment_id = null` (los registros creados por el botón "Servicio realizado"). Botón "Eliminar" en cada línea del detalle del cliente, con confirmación. Endpoint `DELETE /api/purchases/[id]` (permiso `balances`): rechaza 400 si la compra tiene cita asociada, decrementa `users.total_visits` en 1, llama a `recomputeFinancialStatus` y refresca el saldo. Las compras con cita siguen cancelándose desde la agenda (que archiva en `cancelled_appointments`).
- **Referencia opcional en cobros y pagos:** la columna `reference` de `payments` y `supplier_payments` ahora es opcional. Los admins que registran cobros o pagos a proveedores ya no necesitan una referencia (útil para pagos en efectivo). La foto de `supplier_payments` sigue siendo obligatoria. Los clientes nunca llegan a estos endpoints (usan `POST /api/payment-receipts`, sin cambio).
- **Servicios ya realizados sin cita:** nuevo botón "Servicio realizado" en la agenda, en `/dashboard/balances` (por cliente) y en el panel CRM del cliente. Abre `AddServiceDialog` que crea una fila en `service_purchases` con `appointment_id = null` (CXC independiente, recalcula `recomputeFinancialStatus`, incrementa `users.total_visits`). El `GET /api/clients/[id]` ahora hace `LEFT JOIN` con `appointments` y cuenta también compras huérfanas, así el saldo del CRM las refleja.
- **Edición manual de `avg_cost` con auditoría:** nuevo botón "Editar costo" en cada producto de inventario (visible solo con permiso `adjustInventory`). Abre `EditCostDialog` (mini-diálogo con motivo obligatorio) y al confirmar crea un movimiento `kind: "cost_adjust"` en el kardex (`unit_cost_usd` = nuevo valor, `quantity = 0`) además de actualizar `inventory_items.avg_cost`. Visible en el kardex con badge "Costo" púrpura. El guard de `PATCH /api/inventory/items/[id]` exige `canAdjustInventory(session)` y `costNotes` no vacío.
