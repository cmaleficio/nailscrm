...
npm run db:seed:privacy # inserta/actualiza la fila legal_settings de la política de privacidad (placeholders editables luego en /dashboard/legal)
npm run risc:register  # registra /api/risc/events en el stream de Google RISC (requiere RISC_SERVICE_ACCOUNT_JSON_PATH y RISC_RECEIVER_URL)
npm run lint          # ESLint
...
- **Cuentas por cobrar** en `/dashboard/balances`: total adeudado por cliente (calculado en vivo), historial de pagos y registro/borrado de pagos en $ o Bs con la tasa del día del BCV (scrapeada de bcv.org.ve) y abonos parciales. La referencia del pago es opcional (los admins pueden dejar el campo vacío en cobros en efectivo). Incluye la pestaña "Pagos recibidos": cuando el cliente reporta un pago en Bs con captura desde su perfil, el admin lo aprueba (descuenta el saldo) o lo rechaza con motivo. La tasa se refresca diariamente con el endpoint `GET /api/exchange-rate/refresh` (protegido por `CRON_SECRET`), que cron-job.org debe llamar a la URL pública del túnel.
- **Servicios ya realizados sin cita:** botón "Servicio realizado" en la agenda, en Balances (por cliente) y en el panel CRM. Crea una fila en `service_purchases` con `appointment_id = null` (CXC independiente, recalcula `recomputeFinancialStatus` y suma visita al cliente). Desde `/dashboard/balances` se puede eliminar cualquiera de estos registros (con confirmación), decrementando `total_visits` y recalculando el saldo.
- **Menú de navegación público editable:** el admin arma el menú superior de la landing desde `/dashboard/settings/navigation`. CRUD con reordenamiento por posición (fleches), `isActive` (visible) y `openInNewTab`. El `Header` consume `/api/nav-items` y muestra los items activos solo en escritorio (`hidden md:flex`). Seed inicial: Inicio, Políticas de privacidad, Términos y condiciones.
...
- **Inventario** en `/dashboard/inventory`: productos con código (`ACR-001`…), código de barras y foto en un grid, existencias valorizadas (costo promedio ponderado), badge "Stock bajo", kardex de movimientos (salidas y ajustes con motivo) y uso de productos por servicio. Los ajustes requieren el permiso `adjustInventory`. Los esmaltes se organizan por **categoría** (ej: "Esmalte") y **subcategoría** (ej: "Max Glow"), con **máximo de usos** por producto; se muestra el badge "Agotado" (manual o automático al alcanzar el máximo) y los usos consumidos/restantes. Cada producto tiene botón "Editar costo" (requiere `adjustInventory`) que corrige manualmente el `avg_cost` y deja huella con un movimiento `kind: "cost_adjust"` en el kardex.
